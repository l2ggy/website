...
const splitHeroNameLetters = () => {
  ...
  Array.from(originalText).forEach((letter, index) => {
    ...
    span.style.setProperty("--hero-letter-delay", `${Math.floor(Math.random() * 180)}ms`);
  ...
  const randomizeHeroLetters = () => {
    heroName.querySelectorAll(".hero-letter").forEach((span) => {
      span.style.setProperty("--hero-letter-seed", (Math.random() * 2 - 1).toFixed(3));
      span.style.setProperty("--hero-letter-delay", `${Math.floor(Math.random() * 180)}ms`);
    });
  };

  heroName.addEventListener("mouseenter", randomizeHeroLetters);
  heroName.addEventListener("focus", randomizeHeroLetters);
};
