...
export const setupInteractiveGlobe = () => {
  ...
  const draw = () => {
    ...
    const markerYaw = rotateY(torontoVector, Math.cos(yaw), Math.sin(yaw));
    const markerPoint = rotateX(markerYaw, Math.cos(pitch), Math.sin(pitch));
    if (markerPoint[2] > 0) {
      const markerX = center + markerPoint[0] * radius;
      const markerY = center - markerPoint[1] * radius;

      ctx.beginPath();
      ctx.arc(markerX, markerY, Math.max(2.4, size * 0.012), 0, TAU);
      ctx.fillStyle = `${text}f2`;
      ctx.fill();

      ctx.beginPath();
      ctx.arc(markerX, markerY, Math.max(4.6, size * 0.019), 0, TAU);
      ctx.strokeStyle = `${text}70`;
