...
const wrapLongitudeDelta = (value) => {
  const wrapped = ((value + 180) % 360 + 360) % 360 - 180;
  return wrapped;
};

const inEllipse = (lat, lon, centerLat, centerLon, latRadius, lonRadius) => {
  const latDelta = (lat - centerLat) / latRadius;
  const lonDelta = wrapLongitudeDelta(lon - centerLon) / lonRadius;
...
const isLand = (lat, lon) => {
  ...
  return (
    inEllipse(lat, lon, 48, -104, 28, 34) || // North America
    inEllipse(lat, lon, 17, -92, 9, 16) || // Central America
    inEllipse(lat, lon, -17, -60, 25, 17) || // South America
    inEllipse(lat, lon, 11, 20, 29, 24) || // Africa
    inEllipse(lat, lon, 49, 62, 31, 72) || // Eurasia
    inEllipse(lat, lon, 23, 78, 10, 11) || // India
    inEllipse(lat, lon, 65, -42, 12, 13) || // Greenland
    inEllipse(lat, lon, -25, 134, 11, 18) || // Australia
    inEllipse(lat, lon, -4, 121, 8, 26) // Indonesia
  );
};
...
export const setupInteractiveGlobe = () => {
  ...
  const draw = () => {
    ...
    for (let lat = -85; lat <= 85; lat += LAND_STEP) {
      for (let lon = -180; lon <= 180; lon += LAND_STEP) {
        ...
        const shade = 0.5 + point[2] * 0.35;
        ctx.fillStyle = text;
        ctx.globalAlpha = Math.max(0.18, Math.min(0.85, shade));
        ctx.beginPath();
        ctx.arc(projected.x, projected.y, size * 0.0065, 0, TAU);
        ctx.fill();
        ctx.globalAlpha = 1;
      }
    }
