...
const unwrapRing = (ring) => {
  ...
  for (let i = 1; i < ring.length; i += 1) {
    ...
    unwrapped.push([value, lat]);
  }

  return unwrapped;
};

const traceRing = (ctx, ring, shift) => {
...
const createLandMask = (geojson) => {
  ...
  const drawPolygon = (ring) => {
    ...
    const points = unwrapRing(ring.map(([lon, lat]) => [normalizeLongitude(lon), lat]));
    [-360, 0, 360].forEach((shift) => {
      ctx.beginPath();
      traceRing(ctx, points, shift);
      ctx.fill();
    });
  };
  ...
  return {
    width: MASK_WIDTH,
    height: MASK_HEIGHT,
    imageData: ctx.getImageData(0, 0, MASK_WIDTH, MASK_HEIGHT).data
  };
};
...
export const setupInteractiveGlobe = () => {
  ...
  const draw = () => {
    ...
    samples.forEach(({ x, y, vector }) => {
      ...
      const rgba = landMask.imageData[maskIndex] > 120 ? land : ocean;
      const pixelIndex = (y * size + x) * 4;
      outputData[pixelIndex] = rgba[0];
      outputData[pixelIndex + 1] = rgba[1];
      outputData[pixelIndex + 2] = rgba[2];
      outputData[pixelIndex + 3] = rgba[3];
    });
    ...
    if (marker[2] > 0) {
      ...
      ctx.beginPath();
      ctx.arc(x, y, markerSize * 2, 0, TAU);
      ctx.strokeStyle = `${TORONTO_COLOR}aa`;
      ctx.lineWidth = Math.max(1, (window.devicePixelRatio || 1) * 0.9);
      ctx.stroke();

      ctx.beginPath();
      ctx.arc(x, y, markerSize, 0, TAU);
