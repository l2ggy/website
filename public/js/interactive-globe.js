...
export const setupInteractiveGlobe = (markers = []) => {
  ...
  const getZoomScale = () => {
    const styles = getComputedStyle(globe.parentElement || globe);
    const cssValue = Number.parseFloat(styles.getPropertyValue("--globe-zoom-scale"));
    return Number.isFinite(cssValue) && cssValue > 1 ? cssValue : 1.95;
  };
  const getRenderDpr = () => {
