...
const TORONTO_POINT = createPoint(43.6532, -79.3832);
...
export const setupInteractiveGlobe = () => {
  ...
  const draw = () => {
    ...
    const cosPitch = Math.cos(pitch);
    const sinPitch = Math.sin(pitch);
    let marker = rotateY(TORONTO_POINT, cosYaw, sinYaw);
    marker = rotateX(marker, cosPitch, sinPitch);

    if (marker[2] >= 0) {
      const markerX = center + marker[0] * radius;
      const markerY = center - marker[1] * radius;
      const markerRadius = Math.max(2.4, radius * 0.03);
