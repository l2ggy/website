...
export const setupInteractiveGlobe = () => {
  ...
  const draw = () => {
    ...
    const cosYawForward = Math.cos(yaw);
    const sinYawForward = Math.sin(yaw);
    const cosPitchForward = Math.cos(pitch);
    const sinPitchForward = Math.sin(pitch);
    let torontoPoint = latLonToPoint(TORONTO.lat, TORONTO.lon);
    torontoPoint = rotateY(torontoPoint, cosYawForward, sinYawForward);
    torontoPoint = rotateX(torontoPoint, cosPitchForward, sinPitchForward);

    if (torontoPoint[2] > 0) {
      const markerX = center + torontoPoint[0] * radius;
      const markerY = center - torontoPoint[1] * radius;
      const markerRadius = Math.max(2.2, radius * 0.025);

      ctx.beginPath();
      ctx.arc(markerX, markerY, markerRadius * 1.9, 0, TAU);
